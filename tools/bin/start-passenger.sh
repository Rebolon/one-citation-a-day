...
if [ -z "$npm_package_config_settingsfile" ]; then passenger start --port $PORT; else passenger start --meteor-app-settings $npm_package_config_settingsfile --port $PORT; fi
